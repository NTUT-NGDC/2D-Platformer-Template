...
@onready var _mouse_trigger: Node = $KeyTrigger_Mouse
...
func _ready() -> void:
	...
	_mouse_trigger.pressed.connect(func(): print("[測試] KeyTrigger_Mouse（滑鼠右鍵）：pressed"))
	_mouse_trigger.released.connect(func(s): print("[測試] KeyTrigger_Mouse：released，按住了 %.2f 秒" % s))
	print("[測試] 按空白鍵（jump）：角色應該正常跳躍，同時印出 KeyTrigger_Action 的訊息")
	print("[測試] 按住 P 鍵幾秒再放開：應該印出 pressed → 多行 held → released")
	print("[測試] 按住滑鼠右鍵再放開：應該印出 KeyTrigger_Mouse 的 pressed → released")
